...
### 2.将带有自我介绍的星球用户的标签信息导入

```
// 下面为浏览器调试器中复制的请求信息
  
  fetch("https://api.zsxq.com/v2/groups/51122858222824/topics?scope=questions&count=20", {
  "headers": {
...
- 存储用户数组索引和遍历距离的数据结构如果存了所有的分数信息，占用内存。可以维护一个长度固定的有序结构，只保留存储分数前几的几个用户。
